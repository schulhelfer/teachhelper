...
const [componentsSource, persistenceSource, storeSource, planningSource, gradesSource] = await Promise.all([
  readFile(new URL('../src/modules/workspace/components.js', import.meta.url), 'utf8'),
  readFile(new URL('../src/modules/workspace/workspace-persistence.js', import.meta.url), 'utf8'),
...
test('eine neue leere Datenbank ersetzt den aktiven Stand in beiden Persistenzmodi', () => {
  ...
  assert.match(persistenceSource, /buildEmptyDatabaseContainer\(reason = 'create-empty', \{ schoolYearStart = null \} = \{\}\)[\s\S]*?gradeCourseSegments: \[\]/);
  assert.match(persistenceSource, /const isEmptyDatabase = \[[\s\S]*?allowEmpty: isEmptyDatabase/);
  ...
  assert.match(persistenceSource, /this\.store\.buildNewDatabasePublicState\(startYear\)/);
  assert.match(persistenceSource, /buildNewDatabaseSuggestedName\(fileName = this\.fileName \|\| this\.fileHandle\?\.name \|\| ''\)/);
  assert.match(persistenceSource, /async createEmptyWorkspaceFileInDirectory\(directoryHandle, options = \{\}\)[\s\S]*?createEmptyWorkspaceFileInDirectoryNow/);
  assert.match(persistenceSource, /getNewWorkspaceFileHandleInDirectory\(directoryHandle, fileName\)[\s\S]*?getFileHandle\(fileName, \{ create: true \}\)/);
  assert.match(persistenceSource, /assertEmptyWorkspaceDatabaseTarget\(handle, fileName\)[\s\S]*?validateOriginal: async \(original\) => original\.length === 0/);
  assert.match(persistenceSource, /async createEmptyManualDatabase\(options = \{\}\) \{[\s\S]*?const fileName = this\.buildNewDatabaseSuggestedName\(\);[\s\S]*?downloadBytes\(built\.bytes, fileName\);[\s\S]*?await this\.loadBytes\(built\.bytes, 'manual-create-empty'\)/);
