...
const [styles, rootHtml, planningHtml, planningJs, seatplanHtml, serviceWorker] = await Promise.all([
...
test('embedded and dynamic editors retain their event hooks while adopting the shared action design', () => {
  ...
  assert.match(planningJs, /data-grades-entry-save="1" aria-label="Speichern" data-tooltip="Speichern">💾/);
  assert.match(planningJs, /data-grades-entry-cancel="1" aria-label="Abbrechen" data-tooltip="Abbrechen">❌/);
});
