...
test('name learning uses the shared module shell with its own sidebar', () => {
  ...
  assert.match(nameLearningApp, /previousReview: document\.getElementById\('previous-review'\)/);
  assert.match(nameLearningApp, /let previousReview = null;/);
  assert.match(nameLearningApp, /function updatePreviousReviewButton\(\) \{[\s\S]*?mode === 'due' && previousReview && !reviewFeedbackActive/);
  assert.match(nameLearningApp, /function start\(nextMode, preservePreviousReview = false\)/);
  assert.match(nameLearningApp, /start\('due', true\)/);
  ...
  assert.match(nameLearningApp, /refs\.previousReview\.addEventListener\('click', \(event\) => \{ event\.stopPropagation\(\); restorePreviousReview\(\); \}\);/);
  ...
  assert.match(nameLearningCss, /\.main \{ position: relative; display: flex;/);
  assert.match(nameLearningCss, /\.previous-review-action \{ position: absolute;[\s\S]*?top: var\(--app-content-top-space,[\s\S]*?left: 18px;[\s\S]*?width: 38px;/);
  assert.match(nameLearningCss, /\.practice-card\.is-awaiting-next-card #known:disabled, \.practice-card\.is-awaiting-next-card #unknown:disabled \{ cursor: default; \}/);
  assert.doesNotMatch(nameLearningCss, /\.flashcard\.is-awaiting-next-card \.button-row \{ pointer-events: none; \}/);
