...
test('the occurrence mode has a category-aware table marker', () => {
  ...
  assert.match(appSource, /dataset\.gradesEntryOccurrenceCategory = "1"/);
...
test('the occurrence assessment header shows its configured category emoji only when available', () => {
  assert.match(appSource, /function createGradeAssessmentOccurrenceEmoji\(mode, emoji\) \{[\s\S]*?normalizeGradeAssessmentMode\(mode\) !== "homework"[\s\S]*?const normalizedEmoji = normalizeGradeOccurrenceCategoryEmoji\(emoji\);[\s\S]*?if \(!normalizedEmoji\) \{\s+return "";[\s\S]*?grade-assessment-occurrence-icon/);
  assert.match(appSource, /createGradeAssessmentOccurrenceEmoji\(cell\.assessment\.mode, this\.getGradeOccurrenceCategoryEmoji\(cell\.assessment\.occurrenceCategoryId\)\)/);
