...
const [tabs, main, router, bridge, gradesBridge, gradesHtml, gradesApp, nameHtml, nameApp, sharedDialog] = await Promise.all([
  read('../src/shell/tabs.js'), read('../src/app/iframe-module-shell-bindings.js'), read('../src/app/module-message-router.js'), read('../src/app/planning-seatplan-bridge.js'), read('../src/modules/grades/bridge.js'),
...
test('name learning receives its roster through the authenticated grades bridge', () => {
  ...
  assert.match(router, /NAME_LEARNING_STUDENT_SEARCH_REQUEST_EVENT/);
...
test('course pills carry a learner id into participant management and highlight it', () => {
  ...
  assert.match(main, /studentId = Number\(detail\?\.studentId \|\| 0\)/);
