...
export function createDuplicateCheckApp({ root = document } = {}) {
  ...
  function activateTutorialDemo() {
    ...
    tutorialPreviousState = {
      records: lastRecords,
      enabledRules: { ...enabledRules },
      fileSummaryName: ui.fileSummary?.textContent || '',
      fileSummaryVisible: Boolean(ui.fileSummary && !ui.fileSummary.classList.contains('hidden')),
    };
  ...
  function cleanupTutorialDemo() {
    ...
    revokeRecordObjectUrls(demoRecords);
    lastRecords = previousState?.records || [];
    enabledRules = previousState?.enabledRules
      ? { ...previousState.enabledRules }
      : { name: true, size: true, visual: true };
    syncRuleButtons();
    setFileSummary(
      previousState?.fileSummaryVisible && previousState.fileSummaryName
        ? { name: previousState.fileSummaryName }
        : null
    );
...
installTutorialEntryHint(document.getElementById('tutorialButton'), 'duplicate-check', 'DuplikatCheck');
