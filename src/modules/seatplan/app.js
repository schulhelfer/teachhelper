...
import { findNextCourseGradeSeat } from './grade-picker-navigation.js';
        ...
        (function () {
          ...
          appEl.hidden = false;
          const TUTORIAL_DEMO_MODE = new URLSearchParams(window.location.search)
            .get('tutorial-demo') === 'seatplan';
          appEl.dataset.tutorialDemo = TUTORIAL_DEMO_MODE ? '1' : '0';
          const els = {
          ...
          import('../../shared/tutorial-entry-hint.js').then(({ installTutorialEntryHint }) => {
            installTutorialEntryHint(els.tutorialButton, 'seatplan', 'Sitzplan');
          });
          ...
          const state = {
            ...
            pendingCourseSaveRequest: null,
            pendingCourseGradeConfigRequestId: '',
            pendingCourseGradeConfigRequest: null,
          ...
          function canSwitchCourseRoster() {
            return isCourseSeatplanMode()
              && !Number(state.courseContext?.lessonId || 0)
          ...
          function canImportGradeRoster() {
            return !isCourseSeatplanMode() || canSwitchCourseRoster();
          }

          function isCourseRosterSwitchPending() {
            return Boolean(
              state.pendingCourseSwitchCourseId
          ...
          function updateCourseSeatplanUi() {
            ...
            if (els.gradeRosterImportTrigger) {
              els.gradeRosterImportTrigger.disabled = isCourseRosterSwitchPending();
              els.gradeRosterImportTrigger.hidden = active && !canSwitchRoster;
              els.gradeRosterImportTrigger.title = active && !canSwitchRoster
                ? 'Dieser Sitzplan ist bereits mit einem Notenkurs verbunden.'
                : 'Namensliste aus einem Notenkurs importieren';
            }
            if (active && !canSwitchRoster) {
              closeGradeRosterImportMenu();
              if (els.gradeRosterPills) {
                els.gradeRosterPills.hidden = true;
              }
            } else {
          ...
          function closeGradeRosterImportMenu() {
          ...
          function showGradeRosterImportMessage(message) {
            if (!els.gradeRosterImportMenu) return;
            els.gradeRosterImportMenu.replaceChildren();
            const row = document.createElement('div');
            row.className = 'grade-roster-import-message';
            row.textContent = String(message || 'Notenkurse konnten nicht geladen werden.');
          ...
          async function importGradeRosterCourse(courseId) {
            ...
            if (hasUnsavedCourseSeatplanChanges()) {
              const choice = await chooseCourseSwitchAction();
              if (choice === 'save') {
                state.pendingCourseSwitchCourseId = targetCourseId;
                if (!requestCourseSeatplanSave()) {
          ...
          function closeCourseRosterResetDialog(returnValue = '') {
            ...
            if (typeof els.courseRosterResetDialog.close === 'function' && els.courseRosterResetDialog.open) {
              els.courseRosterResetDialog.close(returnValue);
            }
            els.courseRosterResetDialog.removeAttribute('open');
          }
          ...
          function chooseCourseRosterReset() {
            ...
            return new Promise((resolve) => {
              ...
              const finish = (confirmed) => {
                ...
                settled = true;
                cleanup();
                closeCourseRosterResetDialog(confirmed ? 'reset' : '');
                resolve(confirmed);
              };
              const onCancel = (event) => {
                event.preventDefault();
              ...
              const onClose = () => finish(els.courseRosterResetDialog?.returnValue === 'reset');
              const cleanup = () => {
                els.courseRosterResetDialog?.removeEventListener('cancel', onCancel);
                els.courseRosterResetDialog?.removeEventListener('close', onClose);
                els.courseRosterResetDialogConfirm?.removeEventListener('click', onConfirm);
                els.courseRosterResetDialogAbort?.removeEventListener('click', onCancel);
              };
              els.courseRosterResetDialog.addEventListener('cancel', onCancel);
              els.courseRosterResetDialog.addEventListener('close', onClose);
              els.courseRosterResetDialogConfirm?.addEventListener('click', onConfirm);
              els.courseRosterResetDialogAbort?.addEventListener('click', onCancel);
              if (typeof els.courseRosterResetDialog.showModal === 'function') {
                if (!els.courseRosterResetDialog.open) els.courseRosterResetDialog.showModal();
              } else {
                els.courseRosterResetDialog.setAttribute('open', 'open');
              }
              const focusDialog = () => els.courseRosterResetDialogConfirm?.focus({ preventScroll: true });
              if (typeof queueMicrotask === 'function') queueMicrotask(focusDialog);
              else setTimeout(focusDialog, 0);
            });
          }
          ...
          function syncGradeRosterImportHeight() {
            els.gradeRosterPills?.parentElement?.style.removeProperty('height');
          }

          function setGradeRosterImportSurfaceVisible(visible) {
          ...
          function setGradeRosterImportColumns(coursePercent = 50) {
          ...
          function renderGradeRosterPills() {
            ...
            gradeRosterCourses.forEach((course) => {
              ...
              const isSelected = Number(course?.id || 0) === Number(state.courseContext?.courseId || 0)
          ...
          function renderGradeRosterImportCourses(detail) {
            if (!els.gradeRosterImportMenu) return;
            if (String(detail?.requestId || '') !== pendingGradeRosterCoursesRequestId) return;
          ...
          function renderGradeRosterDropdownCourses(courses) {
            if (!els.gradeRosterImportMenu) return;
          ...
          function applyCourseSeatplanContext(detail) {
            ...
            if (gradeConfig && courseGradeConfigMatchesContext(gradeConfig, state.courseContext)) {
              ...
              showMessage('Der Notenkontext ist nicht mehr aktuell. Bitte die Noteneingabe erneut öffnen.', 'warn');
            }
          ...
          function getCourseSeatplanSnapshotKey() {
            if (!isCourseSeatplanMode()) return '';
            const activeSeatIds = [...state.activeSeats].map(String).sort();
            const seats = Object.fromEntries(activeSeatIds.map((seatId) => [seatId, state.seats[seatId] || null]));
            const teacherDistances = [...getTeacherDistanceMap().entries()]
              .map(([studentId, maxDistance]) => [String(studentId), Number(maxDistance)])
              .sort(([left], [right]) => left.localeCompare(right));
            return JSON.stringify({
              gridRows: Number(state.gridRows) || 0,
              gridCols: Number(state.gridCols) || 0,
              activeSeatIds,
              seats,
              ...
              seatScoresHidden: Boolean(state.seatScoresHidden),
            });
          }

          function setCourseSeatplanBaseline() {
            state.courseSeatplanBaseline = getCourseSeatplanSnapshotKey();
          }

          function hasUnsavedCourseSeatplanChanges() {
            return Boolean(
              state.courseSeatplanBaseline
              && state.courseSeatplanBaseline !== getCourseSeatplanSnapshotKey()
            );
          ...
          function requestCourseSeatplanSave() {
            ...
            if (!courseId || !contextToken || !rosterToken) {
              showMessage('Der Kurskontext ist veraltet. Bitte den Sitzplan erneut aus der Planung öffnen.', 'error');
              return false;
            }
          ...
          function closeCourseSwitchDialog(returnValue = '') {
            if (!els.courseSwitchDialog) return;
            if (typeof els.courseSwitchDialog.close === 'function' && els.courseSwitchDialog.open) {
              els.courseSwitchDialog.close(returnValue);
            }
            els.courseSwitchDialog.removeAttribute('open');
          }
          ...
          function chooseCourseSwitchAction() {
            ...
            return new Promise((resolve) => {
              ...
              const finish = (action) => {
                ...
                settled = true;
                cleanup();
                closeCourseSwitchDialog(action || '');
                resolve(action || '');
              };
              const onCancel = (event) => {
                event.preventDefault();
              ...
              const onClose = () => finish(els.courseSwitchDialog?.returnValue || '');
              const cleanup = () => {
                els.courseSwitchDialog?.removeEventListener('cancel', onCancel);
                els.courseSwitchDialog?.removeEventListener('close', onClose);
                els.courseSwitchDialogSave?.removeEventListener('click', onSave);
                els.courseSwitchDialogDiscard?.removeEventListener('click', onDiscard);
                els.courseSwitchDialogCancel?.removeEventListener('click', onCancel);
              };
              els.courseSwitchDialog.addEventListener('cancel', onCancel);
              els.courseSwitchDialog.addEventListener('close', onClose);
              els.courseSwitchDialogSave?.addEventListener('click', onSave);
              els.courseSwitchDialogDiscard?.addEventListener('click', onDiscard);
              els.courseSwitchDialogCancel?.addEventListener('click', onCancel);
              if (typeof els.courseSwitchDialog.showModal === 'function') {
                if (!els.courseSwitchDialog.open) els.courseSwitchDialog.showModal();
              } else {
                els.courseSwitchDialog.setAttribute('open', 'open');
              }
              const focusDialog = () => els.courseSwitchDialogSave?.focus({ preventScroll: true });
              if (typeof queueMicrotask === 'function') queueMicrotask(focusDialog);
              else setTimeout(focusDialog, 0);
            });
          }
          ...
          function handleCourseSeatplanSaveResult(detail) {
            ...
            const courseId = Number(detail.courseId || 0);
            const contextToken = normalizeCourseGradeToken(detail.contextToken);
            const rosterToken = normalizeCourseGradeToken(detail.rosterToken);
            const pending = state.pendingCourseSaveRequest;
            if (
              !pending
              || !requestId
              || requestId !== state.pendingCourseSaveRequestId
              || requestId !== pending.requestId
              || courseId !== pending.courseId
              || contextToken !== pending.contextToken
              || rosterToken !== pending.rosterToken
              || !isCurrentCourseGradeContext(courseId, contextToken)
            ) {
              return;
            }
            state.pendingCourseSaveRequestId = '';
          ...
          function normalizeCourseGradeToken(value) {
            return String(value || '').trim();
          }

          function courseGradeConfigMatchesContext(config, context = state.courseContext) {
            if (!config || typeof config !== 'object' || !context) return false;
            const courseId = Number(config.courseId || 0);
            const contextToken = normalizeCourseGradeToken(config.contextToken);
            const rosterToken = normalizeCourseGradeToken(config.rosterToken);
            return Boolean(
              courseId
              && courseId === Number(context.courseId || 0)
              && contextToken
              && contextToken === normalizeCourseGradeToken(context.contextToken)
              && rosterToken
              && rosterToken === normalizeCourseGradeToken(context.rosterToken)
            );
          }

          function isCurrentCourseGradeContext(courseId, contextToken) {
            return Boolean(
              state.courseContext
              && Number(courseId || 0) === Number(state.courseContext.courseId || 0)
              && normalizeCourseGradeToken(contextToken)
              && normalizeCourseGradeToken(contextToken) === normalizeCourseGradeToken(state.courseContext.contextToken)
            );
          }
          ...
          function normalizeCourseGradeEntries(entries) {
            ...
            sourceEntries.forEach(entry => {
              ...
              if (!studentId || !Number(studentId) || !parsed.valid || seenStudentIds.has(studentId)) {
                valid = false;
                return;
              }
              seenStudentIds.add(studentId);
          ...
          function requestCourseGradeConfig(studentId = '', options = {}) {
            ...
            state.pendingCourseGradeConfigRequest = { requestId, courseId, contextToken };
          ...
          function openFirstCourseGradePicker(attempt = 0) {
            ...
            if (
              els.courseGradePicker?.hidden
              || String(state.courseGradePickerStudentId || '') !== String(input.dataset.studentId || '')
            ) {
              openCourseGradePicker(input);
            }
          }
          ...
          function startCourseGradeMode(values, options = {}) {
            ...
            if (
              !normalizedEntryResult.valid
              || Object.keys(normalizedEntries).some(studentId => !rosterStudentIds.has(String(studentId)))
            ) {
              showMessage('Die Notenkonfiguration enthält kursfremde Einträge und wurde abgelehnt.', 'error');
              return false;
            }
            ...
            renderSeats();
            refreshUnseated();
            requestShellChromeCollapsed(true);
            if (isOccurrenceMode) {
              state.pendingCourseGradeStudentId = '';
              return true;
            }
          ...
          function handleCourseGradeConfigResult(detail) {
            ...
            const requestId = String(detail.requestId || '');
            const courseId = Number(detail.courseId || 0);
            const requestContextToken = normalizeCourseGradeToken(detail.requestContextToken);
            const pending = state.pendingCourseGradeConfigRequest;
            if (
              !pending
              || !requestId
              || requestId !== state.pendingCourseGradeConfigRequestId
              || requestId !== pending.requestId
              || courseId !== pending.courseId
              || requestContextToken !== pending.contextToken
              || !isCurrentCourseGradeContext(courseId, requestContextToken)
            ) {
              return;
            }
            state.pendingCourseGradeConfigRequestId = '';
            ...
            const contextToken = normalizeCourseGradeToken(detail.contextToken);
            const rosterToken = normalizeCourseGradeToken(detail.rosterToken);
            const config = detail.config && typeof detail.config === 'object'
              ? {
                ...detail.config,
                courseId: Number(detail.config.courseId || courseId),
                contextToken: normalizeCourseGradeToken(detail.config.contextToken || contextToken),
              }
              : null;
            if (
              !config
              || Number(config.courseId || 0) !== courseId
              || !contextToken
              || normalizeCourseGradeToken(config.contextToken) !== contextToken
              || !rosterToken
              || normalizeCourseGradeToken(config.rosterToken) !== rosterToken
            ) {
              state.pendingCourseGradeStudentId = '';
              state.courseGradeConfig = null;
            ...
            state.courseContext.contextToken = contextToken;
            state.courseContext.rosterToken = rosterToken;
            if (!courseGradeConfigMatchesContext(config)) {
              state.pendingCourseGradeStudentId = '';
              state.courseGradeConfig = null;
              showMessage('Die Notenkonfiguration ist veraltet oder unvollständig. Bitte erneut öffnen.', 'error');
              return;
          ...
          function markCourseGradeStudentHandled(studentId) {
            const sid = String(studentId || '').trim();
            if (!sid) return false;
            if (!(state.courseGradeHandledStudentIds instanceof Set)) {
              state.courseGradeHandledStudentIds = new Set();
            }
            state.courseGradeHandledStudentIds.add(sid);
            return true;
          }

          function markCourseGradeStudentSkipped(studentId) {
            const sid = String(studentId || '').trim();
            if (!sid) return false;
            if (!(state.courseGradeSkippedStudentIds instanceof Set)) {
              state.courseGradeSkippedStudentIds = new Set();
            }
            state.courseGradeSkippedStudentIds.add(sid);
            return true;
          }

          function clearCourseGradeStudentSkipped(studentId) {
            const sid = String(studentId || '').trim();
            if (!sid) return false;
            return Boolean(state.courseGradeSkippedStudentIds?.delete(sid));
          }

          function isCourseGradeStudentSkipped(studentId) {
            const sid = String(studentId || '').trim();
            if (!sid) return false;
            return Boolean(state.courseGradeSkippedStudentIds?.has(sid));
          }
          ...
          function hideCourseGradePicker(event = null) {
            ...
            courseGradePickerPositionSequence += 1;
            if (courseGradePickerPositionFrame && typeof cancelAnimationFrame === 'function') {
              cancelAnimationFrame(courseGradePickerPositionFrame);
            }
            courseGradePickerPositionFrame = 0;
            ...
            els.courseGradePicker.setAttribute('aria-hidden', 'true');
          ...
          function isCourseGradeStudentDone(studentId) {
            ...
            return Boolean(state.courseGradeHandledStudentIds?.has(sid));
          }
          ...
          function positionCourseGradePicker(input) {
            if (!els.courseGradePicker || !(input instanceof HTMLElement) || !input.isConnected) return null;
            ...
            if (!hasUsableRect(anchorRect) || !hasUsableRect(inputRect)) return null;
            const margin = 8;
            const gap = 10;
            const pickerRect = els.courseGradePicker.getBoundingClientRect();
            if (!hasUsableRect(pickerRect)) return null;
          ...
          function getActiveCourseGradePickerInput() {
            const studentId = String(state.courseGradePickerStudentId || '').trim();
            if (!studentId) return null;
            return getCourseGradeInputOrder().find(
              input => String(input.dataset.studentId || '').trim() === studentId
            ) || null;
          }

          function scheduleCourseGradePickerPosition(options = {}) {
            if (!els.courseGradePicker || els.courseGradePicker.hidden) return;
            const sequence = ++courseGradePickerPositionSequence;
            if (courseGradePickerPositionFrame && typeof cancelAnimationFrame === 'function') {
              cancelAnimationFrame(courseGradePickerPositionFrame);
            }
            const requireStable = options.requireStable === true
              || els.courseGradePicker.dataset.positionReady !== 'true';
            ...
            const run = () => {
              courseGradePickerPositionFrame = 0;
              if (
                sequence !== courseGradePickerPositionSequence
                || els.courseGradePicker?.hidden
                || !isCourseGradeMode()
              ) {
                return;
              }
              const input = getActiveCourseGradePickerInput();
              const positioned = input ? positionCourseGradePicker(input) : null;
              attempt += 1;
              if (positioned) {
                if (positioned.signature === previousSignature) {
                  stableFrames += 1;
                } else {
                  previousSignature = positioned.signature;
                  stableFrames = 0;
                }
                if (!requireStable || stableFrames >= 1 || typeof requestAnimationFrame !== 'function') {
                  els.courseGradePicker.dataset.positionReady = 'true';
                  els.courseGradePicker.style.visibility = 'visible';
                  els.courseGradePicker.setAttribute('aria-hidden', 'false');
                  return;
                }
              }
              if (attempt < 24 && typeof requestAnimationFrame === 'function') {
                courseGradePickerPositionFrame = requestAnimationFrame(run);
              }
            };
            if (typeof requestAnimationFrame === 'function') {
              courseGradePickerPositionFrame = requestAnimationFrame(run);
          ...
          function openCourseGradePicker(input) {
            ...
            els.courseGradePicker.dataset.positionReady = 'false';
            els.courseGradePicker.style.visibility = 'hidden';
            els.courseGradePicker.style.left = '-10000px';
            els.courseGradePicker.style.top = '-10000px';
            els.courseGradePicker.setAttribute('aria-hidden', 'true');
            els.courseGradePicker.textContent = '';
          ...
          function createCourseGradeInput(studentId, label) {
            ...
            input.addEventListener('click', event => {
              ...
              if (
                els.courseGradePicker?.hidden
                || String(state.courseGradePickerStudentId || '') !== sid
              ) {
                openCourseGradePicker(input);
              } else {
          ...
          function buildCourseGradeChanges() {
            const rosterStudentIds = new Set(
              (Array.isArray(state.courseContext?.students) ? state.courseContext.students : [])
                .map(student => String(student?.id || '').trim())
                .filter(Boolean)
            );
            const originalEntries = state.courseGradeOriginalEntries && typeof state.courseGradeOriginalEntries === 'object'
              ? state.courseGradeOriginalEntries
              : {};
            const currentEntries = state.courseGradeEntries && typeof state.courseGradeEntries === 'object'
              ? state.courseGradeEntries
              : {};
            const studentIds = new Set([...Object.keys(originalEntries), ...Object.keys(currentEntries)]);
            const changes = [];
            for (const studentId of studentIds) {
              const sid = String(studentId || '').trim();
              const expectedValue = Object.prototype.hasOwnProperty.call(originalEntries, sid)
                ? Number(originalEntries[sid])
                : null;
              const value = Object.prototype.hasOwnProperty.call(currentEntries, sid)
                ? Number(currentEntries[sid])
                : null;
              if (!sid || !rosterStudentIds.has(sid)) {
              ...
              const expectedValid = expectedValue === null
                || (Number.isInteger(expectedValue) && expectedValue >= 0 && expectedValue <= 15);
              const valueValid = value === null
                || (Number.isInteger(value) && value >= 0 && value <= 15);
              const numericStudentId = Number(sid);
              if (!numericStudentId || !expectedValid || !valueValid) {
                return { valid: false, changes: [] };
              }
              if (expectedValue === value) {
                continue;
          ...
          async function requestCourseGradeSave() {
            ...
            const courseId = Number(state.courseContext?.courseId || 0);
            const contextToken = normalizeCourseGradeToken(state.courseContext?.contextToken);
            const rosterToken = normalizeCourseGradeToken(state.courseGradeConfig?.rosterToken);
            if (
              !courseId
              || !contextToken
              || !rosterToken
              || !courseGradeConfigMatchesContext(state.courseGradeConfig)
            ) {
          ...
          function handleCourseGradeSaveResult(detail) {
            ...
            const contextToken = normalizeCourseGradeToken(detail.contextToken);
            const rosterToken = normalizeCourseGradeToken(detail.rosterToken);
            const pending = state.pendingCourseGradeSaveRequest;
            if (
              !pending
              || !requestId
              || requestId !== state.pendingCourseGradeSaveRequestId
              || requestId !== pending.requestId
              || courseId !== pending.courseId
              || contextToken !== pending.contextToken
              || rosterToken !== pending.rosterToken
          ...
          function applySyncedStudents(detail) {
            ...
            gradeRosterSelectedCourseId = detail.source === 'grades'
              ? Number(detail.gradeCourseId || 0)
              : 0;
            gradeRosterSelectedCourseName = detail.source === 'grades'
          ...
          window.addEventListener('message', (event) => {
            ...
            if (data.type === SEATPLAN_SHELL_LAYOUT_EVENT) {
              ...
              const nextShellCollapsed = detail && detail.collapsed ? 'true' : 'false';
              const shellCollapsedChanged = document.documentElement.dataset.shellCollapsed !== nextShellCollapsed;
              document.documentElement.dataset.shellCollapsed = nextShellCollapsed;
              scheduleCourseGradePickerPosition(shellCollapsedChanged ? { requireStable: true } : {});
              if (detail?.activeTab === 'seatplan') {
                ...
                if (seatplanTabWasActive && isCourseGradeMode()) {
                  requestShellChromeCollapsed(false);
                }
          ...
          window.addEventListener('scroll', () => scheduleCourseGradePickerPosition(), {
            capture: true,
            passive: true,
          });
          ...
          function renderSeats(options = {}) {
            ...
            if (els.courseGradePicker && !els.courseGradePicker.hidden) {
              els.courseGradePicker.style.visibility = 'hidden';
              els.courseGradePicker.dataset.positionReady = 'false';
              els.courseGradePicker.setAttribute('aria-hidden', 'true');
            }
          ...
          if (els.gradeRosterImportTrigger) {
            els.gradeRosterImportTrigger.addEventListener('click', () => {
              ...
              if (gradeRosterCoursesState === 'ready' && gradeRosterCourses.length) {
                renderGradeRosterDropdownCourses(gradeRosterCourses);
                return;
              }
          ...
          if (window.visualViewport) {
            ...
            window.visualViewport.addEventListener('scroll', () => scheduleCourseGradePickerPosition(), { passive: true });
          }
          ...
          if (TUTORIAL_DEMO_MODE) {
            state.students = [
              { id: '01', first: 'Alex', last: 'Beispiel', genderPref: 'm', performanceFlair: 'P1', buddies: ['02'], foes: [] },
              { id: '02', first: 'Sam', last: 'Muster', genderPref: 'w', performanceFlair: 'P1', buddies: ['01'], foes: [] },
              { id: '03', first: 'Kim', last: 'Demo', genderPref: 'w', performanceFlair: 'P2', buddies: [], foes: ['04'] },
              { id: '04', first: 'Robin', last: 'Test', genderPref: 'm', performanceFlair: 'P2', buddies: [], foes: ['03'] },
              { id: '05', first: 'Mika', last: 'Probe', genderPref: 'm', performanceFlair: 'P3', buddies: ['06'], foes: [] },
              { id: '06', first: 'Toni', last: 'Beispiel', genderPref: 'w', performanceFlair: 'P3', buddies: ['05'], foes: [] },
              { id: '07', first: 'Jona', last: 'Muster', genderPref: 'm', performanceFlair: 'P4', buddies: [], foes: [] },
              { id: '08', first: 'Noa', last: 'Demo', genderPref: 'w', performanceFlair: 'P4', buddies: [], foes: [] },
            ];
            state.csvName = 'Tutorial-Beispielklasse';
            state.headers = ['Nachname', 'Vorname'];
            state.gridRows = 3;
            state.gridCols = 5;
            state.activeSeats = new Set([
              '1-1', '1-2', '1-4', '1-5',
            ...
            state.mergedPairs = new Set([pairKey('1-4', '1-5'), pairKey('2-4', '2-5')]);
            state.conditions.teacherDistances = { '03': 2 };
            state.conditions.genderAlternation = true;
            state.lastActionWasPlanSave = true;
            updateCsvStatusDisplay();
          }
          buildGrid();
