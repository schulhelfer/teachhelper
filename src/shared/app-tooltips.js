...
export function installAppTooltips(root = document, options = {}) {
  ...
  function showTooltip(anchor, { persistUntilInteraction = false, onHide = null } = {}) {
    if (!(anchor instanceof HTMLElementCtor) || !anchor.isConnected) return;
    const isSameAnchor = activeAnchor === anchor;
    const shouldPersist = persistUntilInteraction || persistentAnchor === anchor;
    const nextOnHide = typeof onHide === "function"
      ? onHide
      : (isSameAnchor ? activeTooltipOnHide : null);
    if (activeAnchor && activeAnchor !== anchor) {
      restoreAriaDescription(activeAnchor);
      restoreNativeTitles();
    }
    activeAnchor = anchor;
    persistentAnchor = shouldPersist ? anchor : null;
    activeTooltipOnHide = nextOnHide;
  ...
  function hideTooltip(anchor = null) {
    ...
    const onHide = activeTooltipOnHide;
    activeAnchor = null;
    persistentAnchor = null;
    activeTooltipOnHide = null;
  ...
  function positionTooltip() {
    ...
    let cornerPlacement = explicitPlacements
      ? null
    ...
    let placement = cornerPlacement?.placement || placements.find((item) => fits[item]) || placements[0] || "below";
    ...
    const contextMenuRects = [...doc.querySelectorAll(".app-context-menu:not([hidden])")]
      .map((menu) => menu.getBoundingClientRect())
      .filter((rect) => rect.width > 0 && rect.height > 0);
    const overlapArea = (candidateLeft, candidateTop, obstacle) => {
      const width = Math.max(0, Math.min(candidateLeft + tooltipRect.width, obstacle.right) - Math.max(candidateLeft, obstacle.left));
      const height = Math.max(0, Math.min(candidateTop + tooltipRect.height, obstacle.bottom) - Math.max(candidateTop, obstacle.top));
      return width * height;
    };
    const menuOverlap = (candidateLeft, candidateTop) => contextMenuRects.reduce(
      (area, rect) => area + overlapArea(candidateLeft, candidateTop, rect),
      0
    );
    if (contextMenuRects.length && menuOverlap(left, top) > 0) {
      const candidates = [...new Set([placement, ...placements, "above", "below", "right", "left"])]
        .map((candidatePlacement) => {
          let candidateLeft = verticalPlacement.left;
          let candidateTop = anchorRect.bottom + gap;
          if (candidatePlacement === "right") {
            candidateLeft = anchorRect.right + gap;
            candidateTop = anchorRect.top + (anchorRect.height / 2) - (tooltipRect.height / 2);
          } else if (candidatePlacement === "left") {
            candidateLeft = anchorRect.left - tooltipRect.width - gap;
            candidateTop = anchorRect.top + (anchorRect.height / 2) - (tooltipRect.height / 2);
          } else if (candidatePlacement === "above") {
            candidateTop = anchorRect.top - tooltipRect.height - gap;
          }
          candidateLeft = clamp(candidateLeft, margin, Math.max(margin, viewportWidth - tooltipRect.width - margin));
          candidateTop = clamp(candidateTop, margin, Math.max(margin, viewportHeight - tooltipRect.height - margin));
          return { placement: candidatePlacement, left: candidateLeft, top: candidateTop };
        });
      const unobstructed = candidates.find((candidate) => menuOverlap(candidate.left, candidate.top) === 0);
      const best = unobstructed || candidates.reduce((current, candidate) => (
        menuOverlap(candidate.left, candidate.top) < menuOverlap(current.left, current.top) ? candidate : current
      ));
  ...
  function handlePointerOut(event) {
    ...
    if (!anchor) return;
    if (persistentAnchor === anchor) return;
